...
function phaseLabel(state: PaneState): string {
...
function parseReasoning(text: string): { visibleText: string; reasoning: string[] } {
  ...
  const fallbackVisible = visible || text.replace(/<\/?think>/gi, '').trim()
  return { visibleText: fallbackVisible, reasoning }
}
...
export default function AIPane({
  ...
  useEffect(() => {
    ...
    const unsubUpdate = window.electronAPI.onAgentUpdate((updatedTasks) => {
      ...
      setActiveTaskId((prev) => prev && updatedTasks.some((t) => t.id === prev) ? prev : updatedTasks[0].id)
    })
    ...
    const unsubEvent = window.electronAPI.onAgentEvent((event: AgentEvent) => {
      ...
      } else if (event.type === 'step_started' || event.type === 'file_written' || event.type === 'build_started') {
  ...
  const handleRun = async () => {
    ...
    if (mode === 'coding') {
      ...
      try {
        ...
        const codeToApply = extractPrimaryCode(parsed.visibleText)
        if (codeToApply) {
  ...
  const handleApprove = async () => {
    ...
    if (activeTask.mode !== 'plan' && !workspaceRootPath) {
      setPaneState('error')
      onNotify?.('Open a project first.', 'warning')
      return
    }
    try {
      ...
      onNotify?.('Plan approved.', 'success')
    } catch (error: any) {
      setPaneState('error')
    ...
    <aside className="ai-pane">
      <div className="ai-pane-header">
        <div className="ai-pane-title-wrap">
          <h3>AI Assistant</h3>
          <span className={`ai-pane-status ${paneState}`}>{phaseLabel(paneState)}</span>
        </div>
      ...
      <div className="ai-pane-input-wrap">
        ...
        />
        <button
          className={`ai-pane-run-btn ${busy ? 'busy' : ''}`}
        ...
        <section className="ai-pane-section">
          <div className="ai-pane-section-header">
            ...
            <button
              ...
              title="Clear active task selection"
            >
              Clear
            </button>
          </div>
                  ...
                  {activeTask.steps.map((step) => (
                    <li key={step.id} className={`ai-task-step ${step.status}`}>
                      <span className="marker">
                        {step.status === 'completed' ? '✓' : step.status === 'in_progress' ? '…' : step.status === 'failed' ? 'x' : 'o'}
                      </span>
      ...
      {activeTask && activeTask.fileWrites.length > 0 && (
        <section className="ai-pane-section">
          ...
          <div className="ai-pane-filewrites">
            {[...activeTask.fileWrites].reverse().slice(0, 8).map((write) => (
      ...
      {codingResponses.length > 0 && (
        <section className="ai-pane-section">
          ...
          {codingResponses.slice(0, 3).map((resp) => (
            <div key={resp.id} className="ai-coding-response">
              <div className="ai-write-meta">
                <span>Response</span>
                <span>{formatTime(resp.createdAt)}</span>
              </div>
              {resp.reasoning.length > 0 && (
                <div className="ai-reasoning-list">
                  {resp.reasoning.map((reasoning, index) => {
                    const key = `${resp.id}-${index}`
      ...
      {activeTask && (
        <section className="ai-pane-section ai-pane-log-section">
          ...
          <div className="ai-pane-logs">
            ...
            {[...activeTask.logs].slice(-120).map((log) => (
